...
module.exports = {
  config: {
    ...
    countDown: 15,
    role: 0,
    longDescription: "Generate an image using the NanoBanana model with an optional seed.",
    category: "ai-image",
    guide: {
  ...
  onStart: async function({ message, args, event }) {
    ...
    try {
      ...
      tempFilePath = path.join(cacheDir, `nanobanana_output_${Date.now()}.png`);
      ...
      message.reaction("✅", event.messageID);
      await message.reply({
        body: `NanoBanana image generated ✨` + (seed ? ` (Seed: ${seed})` : ''),
