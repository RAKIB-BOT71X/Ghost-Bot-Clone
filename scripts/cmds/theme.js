...
module.exports = {
  ...
  onStart: async function ({ args, message, event, api, getLang, commandName }) {
    ...
    try {
      ...
      const themes = await api.createThemeAI(prompt);
      
      console.log("=== THEME DEBUG ===");
      // The new API returns a single object, not an array
      ...
      for (let index = 0; index < themeArray.length; index++) {
        const theme = themeArray[index];
        ...
        // Use the structure from createThemeAI.js: theme.background_asset.image.url
        if (theme.background_asset?.image?.url) {
          imageUrls.push({ url: theme.background_asset.image.url, name: `theme_${index + 1}_bg.png` });
        }
        
        if (imageUrls.length === 0 && theme.preview_image_urls) {
      ...
      const replyMessage = getLang("preview", themeArray.length, prompt, themeList.trim());
