...
module.exports = {
  config: {
    ...
    countDown: 10,
    role: 0,
    shortDescription: "Love pair v3 - profile card + GIF animation",
    category: "love",
    guide: { en: "{pn}" }
  },
  ...
  onStart: async function ({ api, event }) {
    ...
    try {
      ...
      try {
        ...
        const { data } = await axios.get(apiUrl + "/api/pair/mahmud?user1=" + event.senderID + "&user2=" + match.id + "&style=3", { responseType: "arraybuffer", timeout: 15000 });
      ...
      const gif = LOVE_GIFS[3 % LOVE_GIFS.length];
      try {
        ...
        if (!cardSent) {
          ...
          await api.sendMessage({ body: "Ghost Net Love Pair v3!", attachment: st }, event.threadID);
        }
      } catch {
