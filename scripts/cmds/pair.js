...
𝗟𝗼𝘃𝗲 𝗣𝗲𝗿𝗰𝗲𝗻𝘁𝗮𝗴𝗲: ${lovePercent}% 💙`;
    ...
    // Send animated GIF alongside pair card
    try {
      const gif = PAIR_GIFS[Math.floor(Math.random() * PAIR_GIFS.length)];
      ...
      api.sendMessage({ body: "💕 Ghost Net Love Pair!", attachment: gifSt }, event.threadID);
    } catch {}
    } catch (error) {
      api.sendMessage(
